<?php
/**
 * @var MetaKeyMedia $metaKey
 */

use SweMetaBox\MetaKeyMedia;

$metaValue = $metaKey->getValue(); ?>

<p class="hide-if-no-js">
    <img id="<?php
    echo $metaKey->getKey(); ?>-image"
         src="<?php
         echo wp_get_attachment_image_url($metaValue, 'full'); ?>"
         style="width: 100%; height: auto; border: 0; cursor: pointer; display: <?php
         echo !empty($metaValue) ? 'block' : 'none'; ?>;"/>
</p>
<p class="hide-if-no-js howto" id="set-<?php
echo $metaKey->getKey(); ?>-desc"
   style="display: <?php
   echo !empty($metaValue) ? 'block' : 'none'; ?>;">
    <?php
    _e('Click the image to edit or update'); ?>
</p>

<p class="hide-if-no-js">
    <a href="javascript:"
       id="<?php
       echo $metaKey->getKey(); ?>-remove"
       style="display: <?php
       echo !empty($metaValue) ? 'block' : 'none'; ?>;">
        <?php
...
echo $metaKey->getKey(); ?>-value" name="<?php
echo $metaKey->getKey(); ?>" value="<?php
echo esc_attr($metaValue); ?>"/>
...
<script type="text/javascript">
  jQuery(document).ready(function ($) {
    ...
    jQuery('#<?php echo $metaKey->getKey(); ?>-upload, #<?php echo $metaKey->getKey(
